        ...
        <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-4 mb-8 sm:mb-12 tv:mb-16">
          {['all', 'easy', 'medium', 'hard'].map(f => (
            <button
              ...
              tabIndex={0}
              onClick={() => setFilter(f)}
              className={`px-5 sm:px-6 tv:px-10 py-2 tv:py-4 rounded-full font-bold uppercase tracking-widest text-xs tv:text-base transition-all tv-focusable focus:outline-none focus:ring-4 focus:ring-[#FFD700] ${filter === f ? 'bg-[#FFD700] text-[#06101E] shadow-[0_0_20px_rgba(255,215,0,0.4)]' : 'bg-white/5 text-gray-400 hover:bg-white/10 hover:text-white'}`}
            >
              {f}
            </button>
          ))}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 landscape:grid-cols-2 lg:grid-cols-3 tv:grid-cols-4 gap-5 sm:gap-8 tv:gap-10">
            {filteredQuizzes.map(quiz => (
              <div 
                key={quiz._id} 
                tabIndex={0}
                className="tv-focusable focus:outline-none focus:ring-4 focus:ring-[#FFD700] bg-white/5 border border-white/10 rounded-3xl overflow-hidden backdrop-blur-xl hover:border-[#FFD700]/30 hover:bg-white/10 transition-all duration-300 group flex flex-col cursor-pointer preserve-3d" 
